...
def scores(request):
  if request.method=="POST":
        user=request.user
        feedback_data=Interviewprep.objects.filter(user=user,feedback__isnull=False)
        score=0
        mark=0
        total=10*feedback_data.count()
        for obj in feedback_data:
            if obj.feedback=="😀Excellent Answer,Keep going🥳.":
                mark+=10
            elif obj.feedback=="(●'◡'●)Good Answer,but need some improvement":
                mark+=5
            elif obj.feedback=="⚠Need Improvement,Keep practicing.":
                mark+=1
        score=(mark/total)*10
...
def dele(request):
    if request.method=="POST":
        ...
        user=request.user
        date_time=data.get('delete_date')
        Sum_final=Interviewprep.objects.filter(user=user,created_at=date_time).delete()
